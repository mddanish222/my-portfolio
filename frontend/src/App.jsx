...
function HeroContent() {
  ...
  const { data: resumeData } = useFetch("/settings/resume");
  ...
  const handleDownloadResume = () => {
    if (resumeData && resumeData.value) {
      const link = document.createElement("a");
      link.href = resumeData.value;
      link.download = "Mohammed_Danish_Resume.pdf";
  ...
  return (
    ...
    >
      ...
      <p style={heroDesc}>
        Building scalable web and mobile applications with clean architecture, modern
      ...
      <div style={heroButtons}>
        ...
        <button style={resumeBtn} onClick={handleDownloadResume}>
          📥 Download Resume
        </button>
        <button
          style={secondaryBtn}
      ...
      {showOfflineModal && (
        <div style={offlineModalOverlay}>
          <div style={offlineModalCard}>
            <button
              style={offlineCrossBtn}
              onClick={() => setShowOfflineModal(false)}
              aria-label="Close"
            >
              ✕
            </button>
            <div style={offlineWarningIcon}>📡</div>
            <h3 style={offlineModalTitle}>Please check your internet connection</h3>
          </div>
        </div>
      )}
    </div>
...
function About() {
  ...
  const { data: aboutData } = useFetch("/settings/about_me");
  ...
  if (aboutData && aboutData.value) {
    try {
      parsedAbout = JSON.parse(aboutData.value);
    } catch (e) {
      console.warn("Failed to parse stored about_me JSON");
    }
  }

  const p1 =
    parsedAbout?.paragraph1 ||
    "Full-stack developer who ships production systems end-to-end, across React, Node.js, Spring Boot, Flask, and PostgreSQL/MongoDB. Built and deployed 6+ live applications.";
  const p2 =
    parsedAbout?.paragraph2 ||
    "Passionate BCA student at Seshadripuram College (CGPA: 8.52) specialising in web and app development. Currently a Full Stack Developer at Ontum Education Pvt Ltd, Bengaluru.";

  const stats = [
    { label: "Projects",  value: parsedAbout?.statProjects || "6+" },
    { label: "CGPA",      value: parsedAbout?.statCGPA || "8.52" },
    { label: "PUC Score", value: parsedAbout?.statPUC || "91.16%" },
    { label: "Since",     value: parsedAbout?.statSince || "2024" },
  ];
  ...
  >
    ...
    education.map((edu) => (
      <div key={edu.id || edu._id} style={eduCardBox}>
        <div style={eduCardHeader}>
          <h4 style={eduDegree}>{edu.degree}</h4>
          <span style={eduYearBadge}>{edu.year}</span>
        </div>
        <p style={eduInstitution}>{edu.institution}</p>
        {edu.score && <span style={eduScoreBadge}>{edu.score}</span>}
      </div>
    ))
  ) : null}
</div>
    ...
    <div style={contactWrap}>
      ...
      >
        {[
          ...
          { label: "LinkedIn", value: "linkedin.com/in/mohammed-danish2220" },
        ].map((c) => (
          <div
            key={c.label}
...
const eduScoreBadge = {
  color: "#4ddb8f",
  background: "rgba(30, 180, 100, 0.12)",
  border: "1px solid rgba(30, 180, 100, 0.25)",
  fontSize: "12px",
  fontWeight: "600",
  padding: "3px 10px",
  borderRadius: "6px",
...
const offlineModalCard = {
  ...
  maxWidth: "400px",
  textAlign: "center",
  boxShadow: "0 20px 50px rgba(0, 0, 0, 0.6), 0 0 30px rgba(255, 180, 0, 0.1)",
  position: "relative",
  color: "#fff",
};

const offlineCrossBtn = {
  position: "absolute",
  top: "14px",
  right: "16px",
...
const offlineWarningIcon = {
  fontSize: "44px",
  marginBottom: "12px",
};

const offlineModalTitle = {
  fontSize: "20px",
  fontWeight: "700",
  color: "#FFB400",
  marginBottom: "12px",
